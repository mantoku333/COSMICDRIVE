...
namespace app::test {
    ...
    struct TempoEvent {
        double atBeat = 0.0;
        double bpm = 120.0; // K_DEFAULT_BPM
        double spb = 60.0 / 120.0;
        double atSec = 0.0;
    };
    ...
    public:
        ...
        std::vector<double> activeHoldNextBeats; // lane -> next beat to add combo

        float GetSongTime() const { return songTime; }
        void SetCurrentSongTime(float time);
        int   GetCurrentCombo() const;
        void  AddCombo(int amount); // Explicit combo add
